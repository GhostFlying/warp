...
pub struct ProjectContextModel {
    ...
    /// Local roots currently owned by the metadata-backed refresh path rather than fallback
    /// filesystem scans and watchers.
    #[cfg(feature = "local_fs")]
    metadata_backed_local_roots: HashSet<PathBuf>,
...
impl ProjectContextModel {
        ...
        ctx.spawn(
            ...
            |me, mut res, ctx| {
                for root in res.keys().filter_map(LocalOrRemotePath::to_local_path) {
                    if !me.metadata_backed_local_roots.contains(root) {
        ...
        #[cfg(feature = "local_fs")]
        {
            ...
            ctx.spawn(
                ...
                move |me, res: Result<ProjectRules>, ctx| match res {
                    Ok(rule_files) => {
                        if me.metadata_backed_local_roots.contains(&root_clone)
    ...
    fn register_watcher_for_path(&mut self, path: &Path, ctx: &mut ModelContext<Self>) {
        if self.watched_roots.contains_key(path) || self.metadata_backed_local_roots.contains(path)
        {
            return;
        }
    ...
    fn apply_update_result(
        ...
        if self.metadata_backed_local_roots.contains(path) {
            self.pending_updates.remove(path);
            return;
        }
    ...
    #[cfg_attr(not(feature = "local_fs"), allow(unused_variables))]
    pub fn use_filesystem_fallback_for_root(&mut self, root_path: &Path) {
        #[cfg(feature = "local_fs")]
        self.metadata_backed_local_roots.remove(root_path);
    }
    ...
    pub fn replace_rules_for_metadata_root(
        &mut self,
        ...
        #[cfg(feature = "local_fs")]
        if let Some(local_root) = root_path.to_local_path().map(Path::to_path_buf) {
            self.metadata_backed_local_roots.insert(local_root.clone());
